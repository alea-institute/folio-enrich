...
| `FOLIO_ENRICH_REQUIRE_USER_API_KEY` | `false` | Bring-your-own-key mode. When `true`, a server-stored API key is **never** used to serve enrichment requests, and `PUT /settings` will **not persist** an API key — every visitor supplies their own key, which the browser keeps in `localStorage` and sends per-request only. Set this on public deployments so anonymous users can't spend the operator's key. |
